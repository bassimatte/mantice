...
def build_freesound_description(preset: dict) -> str:
    ...
    description = f"A drone texture generated by MANTICE.\n\n"
    ...
    description += "Per-layer effects include: Chorus (multi-voice LFO-modulated delay for stereo width and organic animation), Flanger (LFO comb sweep with feedback for jet-plane sweeps), Phaser (4-stage all-pass phase shift with configurable rate, depth, and feedback), and Distortion (soft tanh or hard-clip waveshaping).\n\n"
    description += "Global processing includes: 4×-oversampled soft saturation (analog tape-like warmth via tanh waveshaping), an 8-line Feedback Delay Network reverb with Hadamard mixing and per-line damping, Shimmer FX (pitch-shifted feedback tail with selectable intervals from −12 to +24 semitones for ethereal upward bloom), optional binaural beat entrainment (carrier mode with true L/R sine pairs, or detune mode with energy-preserving tremolo), Earth & Air layers (sub-bass tectonic rumble + high-frequency breath turbulence, kept dry post-reverb), 5-band Master EQ (low-cut, bass shelf, lo-mid bell, hi-mid bell, air shelf), feedforward Master Compressor, and true look-ahead Limiter.\n\n"
